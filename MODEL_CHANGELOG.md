...
## The recorder was scoring a different board than the screen - 2026-08-10

NOT a model version — no weight, formula or threshold changed, and `MASTER_MODEL_VERSION` stays 8 with the freeze still running to **2026-11-06**. What changed is which data the scoring harnesses are handed, and whether a broken language corpus is allowed to look healthy. Found while checking whether the Claude API was refreshing the terminal properly.

- **`language.js` was never loaded by any node harness.** `snapshot_scores.js`, `build_signals.js`, `daily_brief.js` and `export_score_report.js` each rebuild the app in a VM from an explicit file list, and every one of those lists omitted the language bundle. `languageCorpus()` therefore returned null in CI, `languageReady()` was permanently false, and the conviction engine's **language vote could never be cast in the recorded track record** — while the browser, which does load it, cast the vote freely. Measured on the 2026-08-10 bundle: **161 of 224 ranks differed** between the snapshot and the live board, top-10 included (PGR 78 vs 77, CI/BMY swapping 9th and 10th). The daily snapshot has been grading a board no user ever saw. All four lists now load every data bundle `index.html` loads, verified board-identical (0 score diffs, 0 rank diffs across all three scoring harnesses).
- **TRACK-RECORD DISCONTINUITY, stated plainly:** snapshots recorded before 2026-08-10 were computed with the language vote structurally absent. The scoring function is unchanged, so they remain v8 — but for names whose conviction carries a language vote, pre- and post-fix snapshots are not strictly like-for-like. This is a recorder repair, not a model change; it is logged here because a track record that silently changes what it measures is worth less than one that says when it did.
- **A dead canonicalisation stage could still be stamped narrative-grade.** The flag was `bool(client) and bool(mapping or raw)` — satisfied by the existence of a single raw theme. On 2026-08-10 the vocabulary call returned `400 … 'Your credit balance is too low to access the Anthropic API'`, canonicalisation produced **nothing**, and the corpus shipped at `canonCoverage: 0.0` still flagged `narrativeGrade: true`. Convergence is counted by exact string match, so that board silently undercounts every narrative on it — the precise failure this file already warned about in a comment while the code permitted it. Grading is now a pure, tested function (`grade_corpus`) requiring **≥25% canonicalisation coverage**; healthy runs measure 0.686–0.756, so the floor rejects a dead stage without punishing normal partial mapping.
- **A degraded run no longer overwrites a healthy board.** The bundle is rebuilt from scratch each run, so one failed canonicalisation replaced a good corpus with an undercounting one — measured alternating 0.735 → 0.0 → 0.756 → 0.0 across four runs, invisibly, because the step is `continue-on-error` and the workflow reported success. A run below the floor now keeps the published bundle and says so; per-company themes are already cached on disk, so nothing collected is lost. The 2026-08-07 healthy corpus (coverage 0.756, 266 canonical themes) has been restored as the published bundle, which is what the guard would have preserved.
- **The dark Radar now states its reason.** `degradedReason` is published in the bundle and rendered, so a missing key, an empty sweep and a billing failure are distinguishable on screen instead of only in CI logs. The old copy blamed n-gram phrase-mining, which stopped being the reason months ago.
- Tests: +9 assertions (1230 total). Section 51 asserts every scoring harness loads every data bundle the browser loads — the guard that makes this class of bug loud instead of silent. Section 52 pins that narrative-grade implies countable coverage, that a withheld board always names its reason, and that the language vote is never cast on a corpus the collector disowned. Python fixtures pin the grading rule at the floor boundary and that an unparseable published bundle degrades to None rather than throwing.

## 8.0.0 — The Graham & Dodd review: the ranking was fighting itself - 2026-08-08

A full read of *Security Analysis* (7th ed., Klarman) against every scored engine. Two defects found in the SCORE path, both of the "the terminal contradicts its own stated rule" class rather than the "model could be better" class — so both ship now, and everything else the review surfaced is recorded in AUDIT.md as v9 candidates rather than half-applied mid-flight. **153 of 224 scores and 216 ranks move**, so v7 snapshots cannot be pooled with v8. Freeze re-armed to **2026-11-06**.

- **The momentum input was signed backwards, and this file said so.** `momentumPart` scored `50 + m1×1.25 + m3×0.55 + day×0.4` — the largest positive coefficient on the **1-month return**. Twenty pages down, the momentum board states in this same file: *"the most recent month exhibits SHORT-TERM REVERSAL, and including it measurably degrades the factor. Most retail momentum screens get this wrong."* The terminal documented the error on one surface and committed it on the surface that sets the ranking (Direction Edge → Tape pillar, 18% of the score, plus a conviction vote). Momentum now reads the **12-1 factor the board already computes**, scored as a **universe percentile** rather than through invented coefficients — momentum is a relative factor and the board's own header says an absolute cutoff "would mean something different in every tape". Fallback for short history is the 3-month window, never the 1-month one.
- **Today's single-day change is out of the trend score entirely.** It is not trend, and a live quote already re-ranks correctly through the PRICE pillar, where a spike makes a name *less* attractive. Keeping a positive same-day term meant momentum was quietly fighting valuation on every tick — measurable: a 50% markdown used to leave Direction Edge flat because the bogus momentum term cancelled the (correct) valuation improvement. It now moves the right way.
- **Missing cash was coerced to zero inside the score engine.** `ev = mcap + (g.debt||0) − (g.cash||0)` and the invested-capital calculation both treated an absent cash balance as *zero cash*, overstating EV and invested capital and therefore understating EV/FCF, EV/adjusted-FCF (24 of the Valuation weight) and ROIC. 22 names carry a null cash balance. This broke the app's own founding invariant in its own scoring path; both now return null so the components drop from the weighted average, exactly as the doctrine requires.
- **Graham's investment/speculative split, display-only** (ch. 4): the master card now publishes an **INVESTMENT** sub-total (business · price · data trust) beside a **SPECULATIVE** one (tape · confluence), so a 78 can no longer hide whether it is a cheap durable business or an expensive one with a hot tape. Graham's instruction is not to discard the speculative component but to *label* it. Tests pin that the two halves partition exactly the covered weight and that the composite lies between them; the headline score is unchanged.
- Board effect: ADBE #1 → #3 (78 → 75) as its one-month tailwind stops counting; the top of the board is now more insurance-weighted, which the existing factor-bet banner already flags.
- Tests: +18 assertions (1230 total). The old momentum test pinned the *order* of three coefficients — the wrong invariant; it now pins **which window carries the sign**, that `quoteChangeOf` is absent from the trend score, and that an identical 12-1 return scores identically no matter what the excluded month did. The live-quote conviction test was narrowed to the sentiment votes it was always about, since Direction Edge legitimately responds to a markdown.
- SBC_MODEL_VERSION 4.5.1 → 4.6.0. App shell v85.
