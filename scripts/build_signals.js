...
/* ---------- 4b. BlackRock 13F deviations (universe names only) ---------- */
let blk13f = prev.blackrock13f || null;
const blkFile = path.join(root, "data", "blackrock.json");
if (fs.existsSync(blkFile)) {
  const B = readJson(blkFile, null);
  const H = B && B.holdings;
  const accn = B && B.f13 && B.f13[0] && B.f13[0].accn;
  if (H && accn && accn !== blk13f) {
    blk13f = accn;
    const dt = H.filed || today;
    (H.new || []).filter(r => r.ticker).forEach(r =>
      events.push({ d: dt, tk: r.ticker, type: "whale", m: 78,
        title: `BlackRock opened a NEW position`, detail: `${(r.value / 1e9).toFixed(1)}B position · 13F for quarter ended ${H.period} (45-day lag applies)` }));
    (H.exits || []).filter(r => r.ticker).forEach(r =>
      events.push({ d: dt, tk: r.ticker, type: "whale", m: 78,
        title: `BlackRock EXITED its position`, detail: `was $${((r.prevValue || 0) / 1e9).toFixed(1)}B · 13F for quarter ended ${H.period} (45-day lag applies)` }));
    (H.adds || []).filter(r => r.ticker && r.sharesChgPct >= 8).slice(0, 8).forEach(r =>
      events.push({ d: dt, tk: r.ticker, type: "whale", m: 66,
        title: `BlackRock ADDED ${r.sharesChgPct.toFixed(0)}% more shares`, detail: `13F quarter ended ${H.period} — index flows possible; size of add is the signal` }));
    (H.trims || []).filter(r => r.ticker && r.sharesChgPct <= -8).slice(0, 8).forEach(r =>
      events.push({ d: dt, tk: r.ticker, type: "whale", m: 66,
        title: `BlackRock TRIMMED ${Math.abs(r.sharesChgPct).toFixed(0)}% of its shares`, detail: `13F quarter ended ${H.period} — index flows possible; size of trim is the signal` }));
  }
}
...
fs.writeFileSync(STATE_FILE, JSON.stringify({ date: today, tickers: stateNow, accessions: accSeen, filingsEmitted, estimates: estSeen, ratingsEmitted, blackrock13f: blk13f }), "utf8");
