...
/* ---------- 4b. whale 13F deviations (universe names only) ---------- */
const whale13f = { ...(prev.whale13f || {}) };
if (prev.blackrock13f && !whale13f.blackrock) whale13f.blackrock = prev.blackrock13f; // migrate old state
const whalesFile = path.join(root, "data", "whales.json");
const WHALE_WEIGHTS = { berkshire: { big: 85, mid: 72 }, blackrock: { big: 78, mid: 66 }, citadel: { big: 58, mid: 52 } };
if (fs.existsSync(whalesFile)) {
  const WD = readJson(whalesFile, null);
  for (const [key, B] of Object.entries((WD && WD.whales) || {})) {
    const H = B && B.holdings;
    const accn = B && B.f13 && B.f13[0] && B.f13[0].accn;
    if (!H || !accn || accn === whale13f[key]) continue;
    whale13f[key] = accn;
    const wgt = WHALE_WEIGHTS[key] || { big: 60, mid: 52 };
    const label = B.label || key;
    const dt = H.filed || today;
    const lagNote = `13F for quarter ended ${H.period} (45-day lag applies)`;
    (H.new || []).filter(r => r.ticker).forEach(r =>
      events.push({ d: dt, tk: r.ticker, type: "whale", m: wgt.big,
        title: `${label} opened a NEW position`, detail: `$${(r.value / 1e9).toFixed(1)}B position · ${lagNote}` }));
    (H.exits || []).filter(r => r.ticker).forEach(r =>
      events.push({ d: dt, tk: r.ticker, type: "whale", m: wgt.big,
        title: `${label} EXITED its position`, detail: `was $${((r.prevValue || 0) / 1e9).toFixed(1)}B · ${lagNote}` }));
    (H.adds || []).filter(r => r.ticker && r.sharesChgPct >= 8).slice(0, 8).forEach(r =>
      events.push({ d: dt, tk: r.ticker, type: "whale", m: wgt.mid,
        title: `${label} ADDED ${r.sharesChgPct.toFixed(0)}% more shares`, detail: lagNote }));
    (H.trims || []).filter(r => r.ticker && r.sharesChgPct <= -8).slice(0, 8).forEach(r =>
      events.push({ d: dt, tk: r.ticker, type: "whale", m: wgt.mid,
        title: `${label} TRIMMED ${Math.abs(r.sharesChgPct).toFixed(0)}% of its shares`, detail: lagNote }));
  }
}

/* ---------- 4bb. insider open-market purchases (SEC Form 4) ----------
   Buys are the signal and sales are mostly noise, so the weights are
   deliberately asymmetric and sales only surface when nobody is buying.
   Emitted once per accession — a filing never changes after it lands. */
const insSeen = { ...(prev.insiders || {}) };
try {
  const IB = typeof INSIDERS !== "undefined" ? INSIDERS : null;
  for (const [tk, r] of Object.entries((IB && IB.tickers) || {})) {
    for (const b of r.buys || []) {
      if (!b.accn || b.planned || b.derivative) continue;
      const key = `${b.accn}|${b.date}`;
      const seen = insSeen[tk] = insSeen[tk] || [];
      if (seen.includes(key)) continue;
      seen.push(key);
      const val = Number.isFinite(b.value) ? ` · $${Math.round(b.value).toLocaleString()}` : "";
      events.push({ d: b.filed || b.date, tk, type: "insider",
        m: r.buyerCount >= 3 ? 88 : b.senior ? 80 : r.buyerCount >= 2 ? 74 : 62,
        title: `Insider bought on the open market: ${b.owner}${b.senior ? " (senior officer)" : ""}`,
        detail: `${b.role || "insider"}${val} on ${b.date}${r.buyerCount >= 2 ? ` · ${r.buyerCount} insiders buying in ${IB.windowDays}d (cluster)` : ""} · own money, not a grant` });
    }
    if (!r.buyerCount) {
      const bigSells = (r.sells || []).filter(s => s.accn && !s.planned);
      const sellers = new Set(bigSells.map(s => s.owner)).size;
      if (sellers >= 3) {
        const key = `sell|${r.lastBuy || ""}|${bigSells[0].accn}`;
        const seen = insSeen[tk] = insSeen[tk] || [];
        if (!seen.includes(key)) {
          seen.push(key);
          events.push({ d: bigSells[0].filed || bigSells[0].date, tk, type: "insider", m: 55,
            title: `${sellers} insiders sold, none bought`,
            detail: "Sales are a weak signal on their own — insiders sell for taxes and diversification — but a one-sided tape is worth noting." });
        }
      }
    }
  }
  for (const tk of Object.keys(insSeen)) if (insSeen[tk].length > 60) insSeen[tk] = insSeen[tk].slice(-60);
} catch (e) { console.error("insider diff skipped:", e.message); }

/* ---------- 4c. narrative regime shifts ----------
   Runs the app's own narrative engine (no duplicated math) and diffs each
   story's measured heat against the last pass. Label changes are regime
   shifts; big heat moves without a label change are momentum warnings.
   First pass records the baseline silently — no invented history. */
const narrPrev = prev.narratives || {};
const narrState = {};
try {
  for (const h of (E.narrativeHeatAll ? E.narrativeHeatAll() : [])) {
    narrState[h.key] = { heat: h.heat, label: h.label };
    const old = narrPrev[h.key];
    if (!old) continue;
    const tag = h.key.toUpperCase().replace(/-/g, " ");
    if (old.label !== h.label) {
      const warmer = h.heat > old.heat;
      events.push({ d: today, tk: tag, type: "narrative", m: 68,
        title: `${h.name} turned ${h.label} (${old.label} → ${h.label})`,
        detail: `heat ${old.heat} → ${h.heat} · ${h.bits.slice(0, 3).join(" · ")}${warmer ? "" : " · money rotating out"}` });
    } else if (Math.abs(h.heat - old.heat) >= 10) {
      events.push({ d: today, tk: tag, type: "narrative", m: 58,
        title: `${h.name} ${h.heat > old.heat ? "heating up fast" : "cooling fast"} (still ${h.label})`,
        detail: `heat ${old.heat} → ${h.heat} in one pass · ${h.bits.slice(0, 3).join(" · ")}` });
    }
  }
} catch (e) { console.error("narrative diff skipped:", e.message); }
...
fs.writeFileSync(STATE_FILE, JSON.stringify({ date: today, tickers: stateNow, accessions: accSeen, filingsEmitted, estimates: estSeen, ratingsEmitted, whale13f, narratives: narrState, insiders: insSeen }), "utf8");
