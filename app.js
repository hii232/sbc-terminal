    ...
    if (!emerging && !shared) {
      return `<div class="card" style="margin-bottom:12px;border-left:3px solid var(--purple)">
        ...
        <div class="sub" style="padding:6px 0;line-height:1.7">Reads earnings-call transcripts and SEC 8-K earnings releases for the largest names, then looks for language <b>spreading between companies</b> — the tell that capital is being redirected before it shows up in any line item. ${meta && meta.generated && meta.narrativeGrade === false
          ? (meta.degradedReason
            ? `A corpus of ${meta.companies} companies was collected on ${escapeHtml(String(meta.generated).slice(0, 10))}, but it is <b>not narrative-grade</b> and is deliberately not shown — ${escapeHtml(String(meta.degradedReason))}. Convergence is counted by exact match, so until themes are canonicalised two companies describing the same strategy in different words never meet, and every narrative on the board would read weaker than it is. Undercounting silently is the one failure this board refuses to ship, so it stays dark until a refresh canonicalises cleanly.`
