...
Owner-earnings stock terminal for a fixed 60-company Core universe plus 60 Extended live-only tickers. The app is static HTML/CSS/JS and is published with GitHub Pages.
...
The terminal intentionally keeps the SEC/SBC-ranked Core universe at exactly 60 stocks. The source of truth is `data/universe.json` and `universe.js`, generated by:

```bash
python scripts/build_universe.py
```

Builds and tests fail if the Core universe is 59, 61, duplicated, missing CIK mappings, or contains an unapproved ticker. FLUT is not part of the current Core 60.

`extended.js` adds 60 more tickers for live quote/search/watchlist coverage. These are labelled `EXTENDED 60` in the app and are not included in SBC rankings, SEC audit counts, owner-earnings P/E rankings, Graham checks, or quality maps until their filing data is built and tested.
...
Current local result: regression tests enforce Core 60 + Extended 60 coverage, golden audit verifies the Core filing layer, browser smoke opens all Core companies plus an Extended ticker and mobile/offline reload, the score/backtest export writes Core score rows, and the SEC alignment report checks Core filing alignment.
